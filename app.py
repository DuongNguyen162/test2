...
class ApplicantSchema(SQLAlchemySchema):
    ...
    country = fields.String(
        description="The object type", enum=[
            x.value for x in Country])
...
def update_applicant_by_id(id):
    ...
    if data.get('created_dttm'):
        get_applicant.created_dttm = datetime.strptime(
            data['created_dttm'], '%m-%d-%Y').date()
...
def add_applicant():
    ...
    data = request.json
    if (data and ('name' in data) and ('email' in data) and ('dob' in data) and (
            'country' in data) and ('status' in data) and ('created_dttm' in data)):
        ...
        try:
            new_applicant = Applicant(
                name, email, dob, country, status, created_dttm)
...
def process_applicant():
    ...
    print(type(date))
    if date % 2 == 0:
        db.session.query(Applicant).filter(Applicant.status).update(
            {Applicant.status: Applicant.status + "processed"})
    else:
        db.session.query(Applicant).filter(Applicant.status).update(
            {Applicant.status: Applicant.status + "failed"})
